...
#line 451
texture2D VHS_InputTexA    { Width = 1920; Height = 1018; Format = RGBA8; };
texture2D VHS_InputTexB    { Width = 1920; Height = 1018; Format = RGBA8; };
texture2D VHS_FeedbackTexA { Width = 1920; Height = 1018; Format = RGBA8; };
texture2D VHS_FeedbackTexB { Width = 1920; Height = 1018; Format = RGBA8; };
texture2D _TapeTex { Width = int(((float)240*(float)1920/(float)1018)); Height = 240; Format = RGBA8; };
#line 457
...
#line 565
float scans = 0.5*(cos( (p.y*float(screenLinesNum)+0.5+t_sl)*2.0*3.1415926535897932) + 1.0);
...
#line 581
const float SLN = float(screenLinesNum) + 0.5; 
#line 583
...
#line 710
const float y = p.y*float2(1920, 1018).y;
const float s = t*2.0;
...
if (VHS_FishEye_Hyperspace){
#line 773
const float prop = float2(1920, 1018).x / float2(1920, 1018).y;
const float2 m = float2(0.5, 0.5 / prop);
const float2 d = (uv*float2(1920, 1018).xy) /float2(1920, 1018).x - m;
...
else uv = (uv*float2(1920, 1018).xy) /float2(1920, 1018).x;
#line 791
uv.y *=  prop;
}
#line 799
...
float2 p = txcoord.xy;
#line 847
float SLN = float(screenLinesNum) + 0.5; 
float SLN_Noise = float(noiseLinesNum) + 0.5; 
float ONE_X = 0.0;
float ONE_Y = 0.0;
#line 854
SLN = float(screenLinesNum) + 0.5; 
SLN_Noise = float(noiseLinesNum) + 0.5; 
if(SLN==0.0) SLN = float2(1920, 1018).y;
#line 858
if(SLN_Noise==0 || SLN_Noise>SLN) SLN_Noise = SLN;
#line 860
ONE_X = 1.0/float2(1920, 1018).x; 
ONE_Y = 1.0/float2(1920, 1018).y;
#line 864
if (VHS_Twitch_V){
...
#line 914
const float ScreenLinesNumX = SLN_Noise * float2(1920, 1018).x / float2(1920, 1018).y;
const float SLN_X = noiseQuantizeX*(float2(1920, 1018).x - ScreenLinesNumX) + ScreenLinesNumX;
pn.x = floor( pn.x * SLN_X )/SLN_X;
#line 918
const float2 pn_ = pn*float2(1920, 1018).xy;
#line 921
...
float fadediff = 0.0;
#line 1010
if(83456 == 0x0A100 || 83456 == 0x0B000) {
fadediff = tex2Dlod(SamplerTape, float4(d,0.0,0.0)).a; 
}
#line 1014
if(83456 == 0x09300 || 83456 >= 0x10000) {
...
if(SLN==0.0) SLN = float2(1920, 1018).y;
#line 1105
float ONE_X = 1.0 / float2(1920, 1018).x;  
ONE_X *= bleedAmount; 
#line 1109
if (VHS_FishEye){
...
const float3 sums = 	t2d((p - float2( 0.5 * float2((1.0 / 1920), (1.0 / 1018)).x, .0)) + float2( (offset - float(taps)) * (ONE_X), 0.0)); +
t2d((p - float2( 0.5 * float2((1.0 / 1920), (1.0 / 1018)).x, .0)) + float2( (float(taps) - offset) * (ONE_X), 0.0)); ;
#line 1463
...
#line 1473
signal += t2d((p - float2( 0.5 * float2((1.0 / 1920), (1.0 / 1018)).x, .0))) * adj;
norm += adj;
signal = signal / norm;
} else {
#line 1478
...
const float one_x = 1.0/float2(1920, 1018).x;
...
p.y=1-p.y;
#line 1601
const float2 p_ = p*float2(1920, 1018);
#line 1603
float ns = 0.0; 
float nt = 0.0; 
float nl = 0.0; 
...
if (VHS_FilmGrain) {
#line 1624
ns += filmGrain((p_-0.5*float2(1920, 1018).xy)*0.5, t, filmGrainPower ) * filmGrainAmount;
}
#line 1627
