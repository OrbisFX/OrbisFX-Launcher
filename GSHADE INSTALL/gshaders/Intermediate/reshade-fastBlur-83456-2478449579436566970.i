...
step *= fast_blur_size/float2(1280/1,720/1);
#line 116
float4 color = 0;
#line 118
const uint steps=5;
...
{
#line 141
return fastBlur(samplerColor, pos, texcoord+(.5/float2(1280/1,720/1)), float2(5,2) );
}
#line 144
float4 fastBlur1b_PS(in float4 pos : SV_Position, in float2 texcoord : TEXCOORD) : COLOR
{
#line 147
return fastBlur(VBlurSampler, pos, texcoord+(.5/float2(1280/1,720/1)), float2(5,2) );
}
#line 150
float4 fastBlur2_PS(in float4 pos : SV_Position, in float2 texcoord : TEXCOORD) : COLOR
{
#line 153
return fastBlur(HBlurSampler, pos, texcoord-(.5/float2(1280/1,720/1)), float2(-2,5) );
}
#line 156
float4 fastBlur3_PS(in float4 pos : SV_Position, in float2 texcoord : TEXCOORD) : COLOR
{
#line 159
return fastBlur(VBlurSampler, pos, texcoord-(.5/float2(1280/1,720/1)), float2(2,5) );
}
#line 162
float4 fastBlur4_PS(in float4 pos : SV_Position, in float2 texcoord : TEXCOORD) : COLOR
{
#line 165
return fastBlur(HBlurSampler, pos, texcoord+(.5/float2(1280/1,720/1)), float2(-5,2) );
}
#line 169
