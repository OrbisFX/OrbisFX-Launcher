...
#line 27
texture GaussianBlurTex < pooled = true; > { Width = 1920; Height = 1018; Format = RGBA8; };
...
for(int i = 1; i < 4; ++i)
{
color += tex2Dlod(GaussianBlurSampler, float4(texcoord + float2(0.0, offset[i] * float2((1.0 / 1920), (1.0 / 1018)).y) * GaussianBlurOffset, 0.0, 0.0)).rgb * weight[i];
color += tex2Dlod(GaussianBlurSampler, float4(texcoord - float2(0.0, offset[i] * float2((1.0 / 1920), (1.0 / 1018)).y) * GaussianBlurOffset, 0.0, 0.0)).rgb * weight[i];
}
}
#line 50
...
for(int i = 1; i < 4; ++i)
{
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord + float2(offset[i] * float2((1.0 / 1920), (1.0 / 1018)).x, 0.0) * GaussianBlurOffset, 0.0, 0.0)).rgb * weight[i];
color += tex2Dlod(ReShade::BackBuffer, float4(texcoord - float2(offset[i] * float2((1.0 / 1920), (1.0 / 1018)).x, 0.0) * GaussianBlurOffset, 0.0, 0.0)).rgb * weight[i];
}
}
#line 136
