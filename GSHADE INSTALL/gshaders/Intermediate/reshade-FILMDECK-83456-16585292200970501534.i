...
for(int y = -1; y <= 1; y++)
{
loc         = float2(x, y) * (float2((1.0 / float2(1280, 720).x), (1.0 / float2(1280, 720).x) * (1280 * (1.0 / 720))));
id          = (x + 1) + (y + 1) * 3;
...
{
float2 texsize = int2(1280 * inscale.x, 720 * inscale.y);
#line 547
float4 uv;
uv.xy = coord * texsize;
#line 551
...
#line 60
texture TexCook < source = "SHADERDECK/LETMECOOK/ross.png"; > { Width = 1280; Height = 720; Format = RGBA8; };
...
for(int i = 1; i < 25; ++i)
{
color.rgb += tex2D(SamplerColor, coord + float2(i * float2((1.0 / 1280), (1.0 / 720)).x, 0.0)).rgb * weight[i];
color.rgb += tex2D(SamplerColor, coord - float2(i * float2((1.0 / 1280), (1.0 / 720)).x, 0.0)).rgb * weight[i];
}
#line 41
...
for(int i = 1; i < 25; ++i)
{
color.rgb += tex2D(SamplerColor, coord + float2(0.0, i * float2((1.0 / 1280), (1.0 / 720)).y)).rgb * weight[i];
color.rgb += tex2D(SamplerColor, coord - float2(0.0, i * float2((1.0 / 1280), (1.0 / 720)).y)).rgb * weight[i];
}
#line 84
return float4(color.rgb, 1);
...
for(int i = 1; i < 18; ++i)
{
color += tex2D(SamplerColor, coord + float2(i * float2((1.0 / 1280), (1.0 / 720)).x, 0.0)) * weight[i];
color += tex2D(SamplerColor, coord - float2(i * float2((1.0 / 1280), (1.0 / 720)).x, 0.0)) * weight[i];
}
#line 108
return color;
...
for(int i = 1; i < 18; ++i)
{
color += tex2D(SamplerColor, coord + float2(0.0, i * float2((1.0 / 1280), (1.0 / 720)).y)) * weight[i];
color += tex2D(SamplerColor, coord - float2(0.0, i * float2((1.0 / 1280), (1.0 / 720)).y)) * weight[i];
}
#line 132
return color;
...
{
#line 182
if (((coord.x + i * float2((1.0 / 1280), (1.0 / 720)).x) > bounds.y  ||
(coord.x - i * float2((1.0 / 1280), (1.0 / 720)).x) < bounds.x)) continue;
#line 185
luma += tex2Dlod(Samplerluma, float4(coord + float2(offset[i] * float2((1.0 / 1280), (1.0 / 720)).x, 0.0) * width, 0.0, 0.0)).x * kernel[i];
luma += tex2Dlod(Samplerluma, float4(coord - float2(offset[i] * float2((1.0 / 1280), (1.0 / 720)).x, 0.0) * width, 0.0, 0.0)).x * kernel[i];
}
#line 189
return luma;
...
(coord.x - i * float2((1.0 / 1280), (1.0 / 720)).x) < bounds.x)) continue;
#line 229
luma += tex2Dlod(Samplerluma, float4(coord + float2(0.0, offset[i] * float2((1.0 / 1280), (1.0 / 720)).y) * width, 0.0, 0.0)).a * kernel[i];
luma += tex2Dlod(Samplerluma, float4(coord - float2(0.0, offset[i] * float2((1.0 / 1280), (1.0 / 720)).y) * width, 0.0, 0.0)).a * kernel[i];
}
#line 233
return luma;
...
(coord.x - i * float2((1.0 / 1280), (1.0 / 720)).x) < bounds.x)) continue;
#line 273
color += tex2Dlod(SamplerColor, float4(coord + float2(offset[i] * float2((1.0 / 1280), (1.0 / 720)).x, 0.0) * width, 0.0, 0.0)).rgb * kernel[i];
color += tex2Dlod(SamplerColor, float4(coord - float2(offset[i] * float2((1.0 / 1280), (1.0 / 720)).x, 0.0) * width, 0.0, 0.0)).rgb * kernel[i];
}
#line 277
return color;
...
(coord.x - i * float2((1.0 / 1280), (1.0 / 720)).x) < bounds.x)) continue;
#line 317
color += tex2Dlod(SamplerColor, float4(coord + float2(0.0, offset[i] * float2((1.0 / 1280), (1.0 / 720)).y) * width, 0.0, 0.0)).rgb * kernel[i];
color += tex2Dlod(SamplerColor, float4(coord - float2(0.0, offset[i] * float2((1.0 / 1280), (1.0 / 720)).y) * width, 0.0, 0.0)).rgb * kernel[i];
}
#line 321
return color;
...
(coord.x - i * float2((1.0 / 1280), (1.0 / 720)).x) < bounds.x)) continue;
#line 361
color += tex2Dlod(SamplerColor, float4(coord + float2(offset[i] * float2((1.0 / 1280), (1.0 / 720)).x, 0.0) * width, 0.0, 0.0)) * kernel[i];
color += tex2Dlod(SamplerColor, float4(coord - float2(offset[i] * float2((1.0 / 1280), (1.0 / 720)).x, 0.0) * width, 0.0, 0.0)) * kernel[i];
}
#line 365
return color;
...
(coord.x - i * float2((1.0 / 1280), (1.0 / 720)).x) < bounds.x)) continue;
#line 405
color += tex2Dlod(SamplerColor, float4(coord + float2(0.0, offset[i] * float2((1.0 / 1280), (1.0 / 720)).y) * width, 0.0, 0.0)) * kernel[i];
color += tex2Dlod(SamplerColor, float4(coord - float2(0.0, offset[i] * float2((1.0 / 1280), (1.0 / 720)).y) * width, 0.0, 0.0)) * kernel[i];
}
#line 409
return color;
...
(coord.x - i * float2((1.0 / 1280), (1.0 / 720)).x) < bounds.x)) continue;
#line 461
luma += tex2Dlod(Samplerluma, float4(coord + float2(0.0, offset[i] * float2((1.0 / 1280), (1.0 / 720)).y) * width, 0.0, 0.0)).x * kernel[i];
luma += tex2Dlod(Samplerluma, float4(coord - float2(0.0, offset[i] * float2((1.0 / 1280), (1.0 / 720)).y) * width, 0.0, 0.0)).x * kernel[i];
}
#line 465
return luma;
...
(coord.x - i * float2((1.0 / 1280), (1.0 / 720)).x) < bounds.x)) continue;
#line 661
luma += tex2Dlod(Samplerluma, float4(coord + float2(i * float2((1.0 / 1280), (1.0 / 720)).x, 0.0) * width, 0.0, 0.0)).a * kernel[i];
luma += tex2Dlod(Samplerluma, float4(coord - float2(i * float2((1.0 / 1280), (1.0 / 720)).x, 0.0) * width, 0.0, 0.0)).a * kernel[i];
}
#line 665
return luma;
...
(coord.x - i * float2((1.0 / 1280), (1.0 / 720)).x) < bounds.x)) continue;
#line 696
luma += tex2Dlod(Samplerluma, float4(coord + float2(0.0, i * float2((1.0 / 1280), (1.0 / 720)).y) * width, 0.0, 0.0)).a * kernel[i];
luma += tex2Dlod(Samplerluma, float4(coord - float2(0.0, i * float2((1.0 / 1280), (1.0 / 720)).y) * width, 0.0, 0.0)).a * kernel[i];
}
#line 700
return luma;
...
for(int i = 1; i < 4; ++i)
{
luma += dot(tex2Dlod(Samplerluma, float4(coord + float2(i * float2((1.0 / 1280), (1.0 / 720)).x, 0.0), 0.0, 0.0)).rgb, float3(0.212395, 0.701049, 0.086556)) * kernel[i];
luma += dot(tex2Dlod(Samplerluma, float4(coord - float2(i * float2((1.0 / 1280), (1.0 / 720)).x, 0.0), 0.0, 0.0)).rgb, float3(0.212395, 0.701049, 0.086556)) * kernel[i];
}
#line 720
return luma;
...
for(int i = 1; i < 4; ++i)
{
luma += tex2Dlod(Samplerluma, float4(coord + float2(0.0, i * float2((1.0 / 1280), (1.0 / 720)).y), 0.0, 0.0)).a * kernel[i];
luma += tex2Dlod(Samplerluma, float4(coord - float2(0.0, i * float2((1.0 / 1280), (1.0 / 720)).y), 0.0, 0.0)).a * kernel[i];
}
#line 745
...
#line 296
texture RT_Swapchain1 { Width = 1280; Height = 720; Format = RGBA16; }; sampler TextureSwapchain1 { Texture = RT_Swapchain1; AddressU = MIRROR; AddressV = MIRROR; AddressW = MIRROR;};
texture RT_Swapchain2 { Width = 1280; Height = 720; Format = RGBA16; }; sampler TextureSwapchain2 { Texture = RT_Swapchain2; AddressU = MIRROR; AddressV = MIRROR; AddressW = MIRROR;};
#line 302
...
grain    *= (pow(GRAIN_N * 0.01, 0.333)) / (dist * (1440.0 / (720 * 1.0)));
#line 441
color.rgb = tex2D(TextureSwapchain2, uv + float2(grain.x / (1280 * (1.0 / 720)), grain.y)).rgb;
...
grain *= (pow(GRAIN_P * 0.01, 0.333)) / (dist * (1440.0 / (720 * 1.0)));
film   = tex2D(TextureSwapchain1, uv + float2(grain.z / (1280 * (1.0 / 720)), grain.y)).rgb;
#line 463
